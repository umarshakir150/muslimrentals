...
| Post listing — city selection shows suggestions, selects a city, sets valid lat/lng | critical | 2026-08-28 | production | TYPECHECKED; DB-verified (82 cities, all with lat/lng); LIVE_SITE_VERIFIED as BROKEN by founder (dropdown showed no suggestions, typed names not recognized, listing unpostable) — real bug found beyond the data fix: CityAutocomplete's module cache treated an empty array as valid forever, and `/cities/all`'s 1-hour Cache-Control masked the fix for existing tabs; both fixed (commit `1612757`, merged to main `20b32e0`), Render deployed live, Netlify build in progress | FIXED_NOT_LIVE_SITE_VERIFIED (2nd round) | Bumped to critical — directly blocks the core posting flow. ai/decisions.md 2026-08-28 entries. Needs a real re-check once Netlify's `20b32e0` deploy finishes — do not assume fixed from code alone this time |
