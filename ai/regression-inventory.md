...
| Post listing — neighbourhood required, real coordinates resolved (not city center) | critical | 2026-08-28 | — | TYPECHECKED; 5 backend Zod tests + frontend unit tests pass; full `next build` production build succeeds | FIXED_NOT_LIVE_SITE_VERIFIED | Schema change (new `Neighbourhood` table) blocked from production by lost MCP access — see ai/decisions.md. Not live yet; do not check this off until deployed AND a real browser post-listing flow is verified |
...
| Owner deletes their own listing (permanent) | critical | 2026-08-28 | — | TYPECHECKED; 5 backend ownership/cleanup tests + 5 frontend dialog tests pass; Security-reviewed (initial CHANGES_REQUIRED on related-data handling, fixed and re-verified) | FIXED_NOT_LIVE_SITE_VERIFIED | Schema change (Conversation.listingId SetNull fix) blocked from production by lost MCP access — see ai/decisions.md. Not live yet |
| Non-owner cannot delete another user's listing | critical | 2026-08-28 | — | Backend test: non-owner request returns 403, delete never called; unauthenticated request returns 401 | FIXED_NOT_LIVE_SITE_VERIFIED | Same deploy blocker as above — code/tests are solid, needs a live re-check (e.g. attempt via devtools/API directly) once deployed |
| Deleted listing disappears from My Listings / browse / map / saved | high | 2026-08-28 | — | Code-reviewed: no shared listing cache exists (each page fetches its own state), so deletion propagates naturally on next fetch; local list pruned immediately via callback | FIXED_NOT_LIVE_SITE_VERIFIED | Same deploy blocker — needs a real live check once deployed, including confirming a saved reference to a deleted listing behaves correctly |
