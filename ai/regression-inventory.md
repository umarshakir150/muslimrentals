...
| Post listing — city selection sets valid lat/lng (not the hardcoded Toronto default) | high | 2026-08-28 | production | TYPECHECKED, DB-verified (Supabase `City` table seeded, 82 rows, `/cities/all` select now includes lat/lng), full stack deployed live (backend on Render, frontend merged to main and Netlify-deployed, commit `5fe4acb`, Netlify deploy `6a9137945ad41b0008ee6a24` `ready`) | FIXED_NOT_LIVE_SITE_VERIFIED | ai/decisions.md 2026-08-28 entries — this session's network egress cannot reach the live domains to click through itself; needs a real browser check to confirm and flip to LIVE_SITE_VERIFIED |
...
| Log in with wrong credentials shows correct error (not "Session expired") | high | 2026-08-28 | production | TYPECHECKED, CODE_REVIEWED, Security-reviewed (APPROVED); deployed live via main merge, commit `5fe4acb`, Netlify deploy `6a9137945ad41b0008ee6a24` `ready` | FIXED_NOT_LIVE_SITE_VERIFIED | Fixed in api.ts (PUBLIC_AUTH_ENDPOINTS) — this session's network egress cannot reach the live domains to click through itself; needs a real browser check to confirm and flip to LIVE_SITE_VERIFIED |
| Google OAuth sign-in | high | never | — | — | NOT_YET_CHECKED | — |
| Forgot / reset password (request → email → set new password → login with new password) | high | 2026-08-28 | production | TYPECHECKED, CODE_REVIEWED, Security-reviewed (APPROVED); full stack deployed live (backend fire-and-forget fix on Render, /reset-password page merged to main, commit `5fe4acb`, Netlify deploy `ready`) | FIXED_NOT_LIVE_SITE_VERIFIED | Needs a real browser check to confirm and flip to LIVE_SITE_VERIFIED. SMTP still unconfigured on Render (SMTP_HOST/USER/PASS unset) — reset emails will not actually send until SMTP is configured; that's a real external-setup gap, not a code bug, and the reset link/page/backend logic can't be fully exercised end-to-end until it's set |
