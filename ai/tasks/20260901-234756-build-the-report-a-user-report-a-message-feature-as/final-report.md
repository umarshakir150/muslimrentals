...
- **Task ID:** 20260901-234756-build-the-report-a-user-report-a-message-feature-as
...
2. BACKEND (new routes, reusing existing patterns -- auth middleware, writeRateLimiter, Zod .strict() validation, matching the existing POST /listings/:id/report exactly in style):
   ...
   - Extend GET /admin/reports to branch on targetType and include the right review context per type (reported user's identity and prior-restriction history for USER; sender identity + the frozen messageSnapshot + a link to the conversation for MESSAGE) -- gated behind the existing ADMIN/MODERATOR role check, no new privilege surface.
...
3. FRONTEND:
   ...
   - Admin dashboard (app/admin/page.tsx) Reports panel: branch on targetType, add a small type chip (Listing/User/Message) per row, and render type-appropriate context instead of the current code that silently assumes every report is listing-shaped (currently shows "Listing: undefined" for anything else) -- reuse the existing admin Users search / account-restriction flow rather than building a new inline restriction button.
...
5. Out of scope, do not implement: any automated restriction/content-removal action (all account restrictions stay behind the existing ADMIN-only restriction flow with a human decision-maker), a report-priority auto-flagging system (noted as a future enhancement, not needed for this pass), a "block user" feature.

This closes a real, named Trust & Safety gap (company/architecture.md's "Known weaknesses": only listings are reportable today) and is an explicit ai/roadmap.md "Next" priority. Founder has explicitly approved this exact scope including the prior-interaction constraint above (see ai/decisions.md and the approval record appr_ab411474-e4d4-4742-9ed8-f28a463d1d5d for the full decision). Founder requires QA, Security, AND Trust & Safety review before this is considered ready -- make sure the plan includes trust_safety review of the actual implemented code (not just the design), since the prior review was of a version that was never merged and never re-verified. Do not merge to main, do not push to main, do not publish anything live -- leave the reviewed branch for the founder to open a PR from and review a real Netlify preview build before merge.
...
Task complete. Agents involved: trust_safety, backend, frontend, qa, security, integrator. 1 correction cycle(s) used.
...
- Review/merge the INTEGRATED branch "agents/20260901-234756-build-the-report-a-user-report-a-message-feature-as/integration" at /home/user/muslimrentals/orchestrator/.worktrees/20260901-234756-build-the-report-a-user-report-a-message-feature-as-integration — this is the reviewed, mergeable result. The individual implementer branches below are its inputs, already folded in; they don't need separate merging.
- Implementer branch "agents/20260901-234756-build-the-report-a-user-report-a-message-feature-as/backend" (backend) at /home/user/muslimrentals/orchestrator/.worktrees/20260901-234756-build-the-report-a-user-report-a-message-feature-as-backend — not auto-merged by the orchestrator.
- Implementer branch "agents/20260901-234756-build-the-report-a-user-report-a-message-feature-as/frontend" (frontend) at /home/user/muslimrentals/orchestrator/.worktrees/20260901-234756-build-the-report-a-user-report-a-message-feature-as-frontend — not auto-merged by the orchestrator.
